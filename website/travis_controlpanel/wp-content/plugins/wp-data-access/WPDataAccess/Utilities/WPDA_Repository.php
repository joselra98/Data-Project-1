...
namespace WPDataAccess\Utilities {
	...
	class WPDA_Repository {
					...
					if ( $this->run_script( $value[0], '_new' ) ) {
						...
						if ( $table_diff !== null ) {
							...
							$same_cols = $wpdb->get_results(
									$wpdb->prepare( $sql_check_table, $wpdb->dbname, $table_name, $wpdb->dbname, $bck_table_name )
									, 'ARRAY_A' );
		...
		public function remove_backup( $backup_date ) {
			global $wpdb;
			$backup_date_sanitized = str_replace( '`', '', $backup_date );

			$suppress  = $wpdb->suppress_errors( true );
			$extension = WPDA_Restore_Repository::BACKUP_TABLE_EXTENSION;
			foreach ( self::CREATE_TABLE as $key => $value ) {
				$wpdb->query( "drop table `{$wpdb->prefix}{$key}{$extension}{$backup_date_sanitized}`" );
			}
			$wpdb->suppress_errors( $suppress );
		}
