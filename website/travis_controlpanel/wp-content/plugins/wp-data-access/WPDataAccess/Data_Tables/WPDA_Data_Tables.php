...
class WPDA_Data_Tables
{
    ...
    )
    {
        ...
        // Add premium extention support
        $this->extention_wizard( $pub_extentions );
    ...
    protected function extention_wizard( $pub_extentions )
    {
    }
    ...
    protected function get_labels( $pub_format )
    {
        ...
        if ( isset( $pub_format_json['pub_format']['column_labels'] ) ) {
            return $pub_format_json['pub_format']['column_labels'];
        } else {
    ...
    {
        $dataTablesClass = str_replace( [ ',', 'default' ], [ ' ', 'display' ], $pub_styles );
    ...
    public function get_data()
    {
        ...
        if ( '' === $pub_id && '' === $table_name ) {
            ...
            foreach ( $rows as $row ) {
                ...
                for ( $i = 0 ;  $i < sizeof( $row ) ;  $i++ ) {
                    $row[$i] = str_replace( [ '<script>', '</script>' ], [ '&lt;script&gt;', '&lt;/script&gt;' ], $row[$i] );
                }
    ...
    private function qb_criteria( $data, $crit )
    {
    }
    
    private function qb( $where, $pub_format )
    {
    }
