    ...
    public function show( $allow_save = true, $add_param = '' )
    {
        ...
        foreach ( $this->fieldsets as $fieldset_title => $fieldset ) {
            ...
            if ( '' !== $fieldset_title ) {
                ...
                if ( $expandable && '' !== $id ) {
                    echo  '<div class="wpda-fieldset-expand" onclick="expandFieldset(\'' . esc_attr( $id ) . '\')">' . '<i id="wpda_field_expand_' . esc_attr( $id ) . '" class="wpda-fieldset-expand material-icons">' . $expandable_icons . '</i>' . '<span class="wpda-fieldset-expand">' . esc_html( $fieldset_title ) . '</span>' . '<input type="hidden" id="wpda_fieldset_expand_' . esc_attr( $id ) . '" name="wpda_fieldset_expand_' . esc_attr( $id ) . '" value="' . esc_attr( $expandable_state ) . '" />' . '</div>' ;
                } else {
