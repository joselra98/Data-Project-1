...
class WPDA_Publisher_List_Table extends WPDA_List_Table
{
    ...
    public function column_default( $item, $column_name )
    {
        ...
        if ( 'pub_table_options_searching' === $column_name || 'pub_table_options_ordering' === $column_name || 'pub_table_options_paging' === $column_name ) {
    ...
    public function column_default_add_action( $item, $column_name, &$actions )
    {
        ...
        $copy_form = "<form" . " id='copy_form{$form_id}'" . " action='?page=" . esc_attr( $this->page ) . "&table_name=" . esc_attr( $this->table_name ) . "'" . " method='post'>" . $this->get_key_input_fields( $item ) . "<input type='hidden' name='action' value='copy' />" . "<input type='hidden' name='_wpnonce' value='{$wp_nonce}'>" . $this->page_number_item . "</form>";
        ?>
    ...
    {
        global  $wpdb ;
        $db_pub_name = $wpdb->get_results( $wpdb->prepare( 'select pub_name from ' . WPDA_Publisher_Model::get_base_table_name() . ' where pub_id = %d', [ $pub_id ] ), 'ARRAY_A' );
        if ( $wpdb->num_rows !== 1 ) {
            wp_die( __( 'ERROR: Publication not found', 'wp-data-access' ) );
        }
        $pub_name = $db_pub_name[0]['pub_name'];
        $i = 2;
        $query = "select 'x' from " . WPDA_Publisher_Model::get_base_table_name() . ' where pub_name = %s';
